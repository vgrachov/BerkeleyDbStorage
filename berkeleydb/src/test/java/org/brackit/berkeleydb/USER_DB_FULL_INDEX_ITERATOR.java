...
public class USER_DB_FULL_INDEX_ITERATOR {

	private static final Logger logger = Logger.getLogger(USER_DB_FULL_INDEX_ITERATOR.class);
	
	@Test
	public void fullIndexIterator(){
		Schema schema = Catalog.getInstance().getSchemaByDatabaseName("USER_TABLE");
		ITupleCursor cursor = new FullIndexIterator(schema.getColumns()[1]);
		cursor.open();
		Tuple tuple = null;
